...
def make_box(coords, width_km=25, height_km=25):
    ...
    '''
    if width_km == None or height_km == None:
        return None
    ...
    for i, gt in enumerate(tracks):
        ...
        if i % 2 == 0:
            ...
            if polygon is not None:
                ...
                lats = [(atl08.df['latitude'].min() + atl08.df['latitude'].max())/2]
        ...
        for n, lat in enumerate(lats):
            if lat is not None:
                if i % 2 == 0:
                    polygon = make_box((coords[0],lat), width, small_box)
                ...
                atl03_temp = atl03.df.copy()
                atl08_temp = atl08.df.copy()
                ### CONTINUE FROM HERE TO ALLOW FOR NO WIDTH/HEIGHT,
            ### THEN NO SMALL_BOX,
            ### THEN WIDTH/HEIGHT W/OUT SMALL_BOX,
            ### THEN SMALL_BOX W/O WIDTH/HEIGHT
